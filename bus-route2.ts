export function numBusesToDestination(routes: number[][], source: number, target: number): number {
  ...
  const routeConnections: number[][] = stopsToRoutes.filter((routes) => routes.length > 1);
  const availableStartingRoutes = stopsToRoutes[source];
  const availableEndingRoutes = stopsToRoutes[target];

  return findShortestPath(routeConnections, availableStartingRoutes, availableEndingRoutes);
}
...
function findShortestPath(routeConnections: number[][], availableStartingRoutes: number[], availableEndingRoutes: number[]): number {
  let count = 1000;
...
function dfs(connections: Set<number>[], start: number, end: number): number {
  ...
  function traverse(): number[] {
    ...
    while (queue.length > 0) {
      ...
      if (!visited[route]) {
        visited[route] = true
        if (!connections[route]) break; // the rotue is isolated
